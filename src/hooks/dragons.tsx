...
/**
 * Fetches a single dragon given dragon contract data.
 * @param accountState 
 * @param dispatch 
 * @param dragon 
 */
async function fetchDragonData(accountState: any, dispatch: any, resetContracts: any, dragon: any): Promise<DragonData> {
  const [
    name,
    maxHealth,
    health,
  ...
  const healthPercent = new BN(health).mul(new BN('10000')).div(new BN(maxHealth)).div(new BN('100')).toString();

  const realSecondsUntilBreed = Number(secondsUntilBreed) > 0 ? new BN(2 *  (BREED_COOLDOWN_INIT * (2 ** (Number(breedCount) + 1))) - Number(secondsUntilBreed)).toString(): '0';
  const realSecondsUntilAttack = Number(secondsUntilAttack) > 0 ? new BN(attackCooldown).mul(new BN('2')).sub(new BN(secondsUntilAttack)).toString(): '0';
  const realSecondsUntilUpgrade = Number(secondsUntilUpgrade) > 0 ? new BN((2 * UPGRADE_COOLDOWN) - Number(secondsUntilUpgrade)).toString(): '0';
...
async function updateDragonState(accountState: any, dispatch: any) {
  ...
  try {
    const dragons = await Promise.all(contracts.Dragons.map(async (dragon: any) => {
      try {
        const dragonData = await fetchDragonData(accountState, dispatch, resetContracts, dragon);
        return dragonData;
      } catch (err) {
